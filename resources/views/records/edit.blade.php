@extends('layouts.app')
@section('title', $patient->name . 'の記録番号:' . $record->id . 'の編集')
@section('content')         
    <div class="row mt-3">
        <h1 class="col-sm-12 text-center text-success pb-1 mt-5">{{ $patient->name }}の記録番号: {{ $record->id }}の編集</h1>
    </div>
    <div class="col-sm-6 offset-sm-3 mt-3 mb-5">
        {!! Form::open(['route' => ['records.update', $patient->id, $record->id], 'files' => true, 'method' => 'PUT']) !!}
        <!-- 1行 -->
